...
export default function Home() {
  ...
  // Ajustar la posición libre para evitar el área del botón
  const encontrarPosicionLibre = (width: number, height: number): { top: number; left: number } | null => {
    ...
    // Área reservada para el botón (70px desde la parte superior)
    const botonAreaTop = 70
    const botonAreaBottom = botonAreaTop + 50 // Asumiendo que el botón tiene ~50px de altura

    let intentos = 0
    while (intentos < 100) {
      const top = Math.random() * (containerHeight - botonAreaBottom) + botonAreaBottom // Evitar el área del botón
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    setStartPos({
      x: touch.clientX - bolita.x,
      y: touch.clientY - bolita.y
    })
  }
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    if (isDragging) {
      ...
      const endX = touch.clientX - startPos.x
      const endY = touch.clientY - startPos.y
  ...
  return (
    ...
    >
      <Button
        ...
        className="z-30 bg-white text-gray-800 hover:bg-gray-200 transition-colors duration-300 shadow-lg mb-8"
      >
        Ando estresada
      </Button>
      ...
      {mensajes.map((mensaje) => (
        <div
          key={mensaje.id}
          className={`absolute p-4 rounded-md shadow-lg transition-all duration-300 ${mensaje.color} hover:scale-105 hover:shadow-xl select-none max-w-full h-full flex items-center justify-center z-20`}
          style={{
      ...
      <Button
        ...
        className="fixed bottom-4 right-4 z-30 bg-red-400 hover:bg-red-500 transition-colors duration-300 shadow-lg"
      >
        <X className="mr-2 h-4 w-4" /> Limpiar
      </Button>

      <div
        className="absolute w-5 h-5 rounded-full bg-red-500 shadow-lg cursor-grab active:cursor-grabbing touch-none"
        style={{
          left: `${bolita.x}px`,
          top: `${bolita.y}px`,
